...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            model += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias), #input == 64*4
...
class Bottle2neck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, baseWidth=26, scale=4, stype='normal'):
        """ Constructor
        Args:
            inplanes: input channel dimensionality  输入的维度
            planes: output channel dimensionality   输出的维度
            stride: conv stride. Replaces pooling layer.    卷积的步长
            downsample: None when stride = 1
            baseWidth: basic width of conv3x3  卷积核的大小
            scale: number of scale.
            type: 'normal': normal set. 'stage': first block of a new stage.
        """
        super(Bottle2neck, self).__init__()

        width = int(math.floor(planes * (baseWidth / 64.0)))  # width=26
    ...
    def forward(self, x):
        ...
        spx = torch.split(out, self.width, 1)  # 切割份
        for i in range(self.nums):
